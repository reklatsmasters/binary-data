...
module.exports = function array(type, length, lengthType = 'count') {
  ...
  function encode(items, wstream, context) {
    ...
    if (isLengthInBytes) {
      const requiredLength = items.reduce(
        (counter, item) => counter + common.encodingLength(item, type),
        0
      )
    ...
    items.forEach(item => {
      common.encode(item, wstream, type)
      encode.bytes += common.encode.bytes
    })
  }
...
function decodeBytes(type, lengthBytes, rstream, items = []) {
  ...
  while (bytes < lengthBytes) {
    items.push(common.decode(rstream, type))
    bytes += common.decode.bytes
  }
...
function decodeCount(type, length, rstream) {
  ...
  const items = Array.from(new Array(length)).map(() => {
    const item = common.decode(rstream, type)
    bytes += common.decode.bytes
