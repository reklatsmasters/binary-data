...
class Reserved {
  ...
  encode(_, wstream, context) {
    ...
    for (let i = size; i > 0; --i) {
      common.encode(0, wstream, this.type)
      this.encode.bytes += common.encode.bytes
    }
  }
  ...
  decode(rstream, context) {
    ...
    for (let i = size; i > 0; --i) {
      common.decode(rstream, this.type)
      this.decode.bytes += common.decode.bytes
    }
  }

  encodingLength(value) {
    return common.encodingLength(value, this.type) * this.size
  }
}
