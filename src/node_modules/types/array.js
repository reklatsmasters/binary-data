...
function array(type, length, lengthType = 'count') {
  ...
  function decode(rstream, meta) {
    ...
    if (isnum) {
      ...
      expectedSize = length.decode(rstream, meta)
    } else if (isfunc) {
      expectedSize = length(meta.context)
    }

    checkArraySizeType(expectedSize)

    if (isLengthInBytes) {
      return decodeBytes(type, expectedSize, rstream, meta)
    }

    return decodeCount(type, expectedSize, rstream, meta)
  }
...
function decodeBytes(type, lengthBytes, rstream, meta) {
  const items = []
  const before = meta.bytes
  let bytes = 0

  while (bytes < lengthBytes) {
    items.push(decodeCommon(rstream, type, meta))
    bytes = meta.bytes - before
  }
...
function decodeCount(type, length, rstream, meta) {
  const items = new Array(length)

  for (let i = 0; i < length; ++i) {
    items[i] = decodeCommon(rstream, type, meta)
  }
