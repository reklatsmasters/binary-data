...
function decodeNullString(encoding) {
  ...
  return function decode(rstream, meta) {
    let bytes = 0
...
function decodeSizePrefixedString(type, encoding) {
  // eslint-disable-next-line func-names
  return function decode(rstream, meta) {
    const size = type.decode(rstream, meta)
...
function encodeCallback(callback, encoding) {
  // eslint-disable-next-line func-names
  return function encode(value, wstream, context) {
    encode.bytes = 0
...
function decodeCallback(callback, encoding) {
  // eslint-disable-next-line func-names
  return function decode(rstream, meta) {
    const size = callback(meta.context)
